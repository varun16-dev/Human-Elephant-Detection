# Elephant Alert System - Setup Guide

Department of AI & ML, Sri Sairam College of Engineering

## Overview

This alert system adds comprehensive notification capabilities to the existing elephant detection system. It provides:
...
- **WhatsApp Alerts**: Official WhatsApp Business API integration with image attachments
- **Alert History**: Complete tracking of all confirmed elephant events
- **Dashboard UI**: Full management interface for alerts and recipients
...
### 3. Configure WhatsApp Recipients

Edit `whatsapp_recipients.json` to add your alert recipients:

```json
[
  {
    "id": "recipient_001",
    "name": "Forest Officer 1",
    "role": "FOREST_OFFICER",
    "phone": "+91XXXXXXXXXX",
    "enabled": true,
    "subscribed": true
...
**Important:** Recipients must have consented to receive WhatsApp alerts per WhatsApp's policies.
...
4. **Manage WhatsApp Recipients**: View configured contacts
5. **Test Notifications**: Send test alerts to verify configuration
6. **Reset Cooldown**: For testing purposes
...
5. Notifications sent to:
   - Forest Officers (Web Push + WhatsApp)
   - Villagers (Web Push + WhatsApp)
...
An elephant has been CONFIRMED by the monitoring system.
...
2. Click "Test to Forest Officers" or "Test to Villagers"
3. Check WhatsApp for test message (marked as TEST)
...
- Verify recipient phone numbers are in correct format (+91XXXXXXXXXX)
...
- WhatsApp recipient phone numbers are masked in dashboard
- Use official WhatsApp Business API only
- Ensure recipient consent for WhatsApp alerts
...
- `whatsapp_recipients.json` - WhatsApp contacts
...
- `static/css/main.css` - No changes (not modified)
...
- Follows WhatsApp messaging policies
